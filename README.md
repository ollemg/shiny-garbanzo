...
- 🎯 Entry points to execute your program using `python -m <shiny_garbanzo>` or `$ shiny_garbanzo` with basic CLI argument parsing.
...
---
# shiny_garbanzo Flask Application

Awesome shiny_garbanzo created by ollemg
...
```bash
git clone https://github.com/ollemg/shiny-garbanzo shiny_garbanzo
cd shiny_garbanzo
make install
```
...
```bash
shiny_garbanzo create-db   # run once
shiny_garbanzo populate-db  # run once (optional)
shiny_garbanzo add-user -u admin -p 1234  # ads a user
shiny_garbanzo run
```
