...
original_author="ollemg"
original_name="shiny_garbanzo"
original_urlname="shiny-garbanzo"
original_description="Awesome shiny_garbanzo created by ollemg" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/shiny_garbanzo "${name}"
...
then
    ...
    echo "then run 'shiny_garbanzo' or 'python -m shiny_garbanzo'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'shiny_garbanzo' or 'python -m shiny_garbanzo' or 'poetry run shiny_garbanzo'"
fi
